...
const Dashboard: React.FC = () => {
  ...
  const setPayrollDraft = useStore(s => s.setPayrollDraft);
    ...
    <div className="p-4 md:p-8 max-w-7xl mx-auto space-y-6 md:space-y-10 pb-20">
      <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6 bg-white dark:bg-slate-800/30 p-6 md:p-8 rounded-2xl border border-slate-200 dark:border-slate-700/50 backdrop-blur-sm shadow-sm transition-colors">
        <div className="flex flex-col gap-2">
          <h1 className="text-slate-900 dark:text-white text-3xl md:text-4xl font-black tracking-tight leading-tight">Financial Overview</h1>
          <p className="text-slate-500 dark:text-slate-400 font-medium text-sm md:text-base">Comprehensive payroll insights for Oct 2023 cycle.</p>
        </div>
        <button 
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
        <StatCard title="Total Monthly Spend" value="$142,500" trend="+2.4%" color="primary" />
        <StatCard title="Active Workforce" value="48" trend="+1 New Hire" color="emerald" />
        <StatCard title="Compliance Health" value="100%" trend="Verified" color="blue" className="sm:col-span-2 lg:col-span-1" />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-8">
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-700/50 overflow-hidden flex flex-col shadow-sm transition-colors">
          <div className="p-5 md:p-6 border-b border-slate-100 dark:border-slate-800/50 flex items-center justify-between">
             <h3 className="font-black text-slate-900 dark:text-white uppercase tracking-widest text-[10px] md:text-xs">Payroll Readiness</h3>
          ...
          <div className="p-5 md:p-6 space-y-5 flex-1">
            <CheckItem label="Tax ID & Compliance Status" sub="RC1234567 • Fully Compliant" completed />
            <CheckItem label="Bank Account Connectivity" sub="Verified Corporate Account" completed />
            <CheckItem label="Review Pending Adjustments" sub="3 items awaiting approval" action="View" onClick={() => navigate('/payroll/approvals')} />
          </div>
        </div>

        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-700/50 overflow-hidden flex flex-col shadow-sm transition-colors">
           <div className="p-5 md:p-6 border-b border-slate-100 dark:border-slate-800/50 flex items-center justify-between">
             <h3 className="font-black text-slate-900 dark:text-white uppercase tracking-widest text-[10px] md:text-xs">Recent History</h3>
             <button onClick={() => navigate('/payroll/history')} className="text-[10px] font-black text-primary uppercase hover:underline">Full History</button>
          </div>
          <div className="p-5 md:p-6 divide-y divide-slate-100 dark:divide-slate-800/50">
            {Array.isArray(history) && history.length > 0 ? history.slice(0, 4).map((run) => (
              <div key={run.id} className="flex items-center justify-between py-3 first:pt-0 last:pb-0">
                ...
                <div className="text-right shrink-0">
                  <span className="text-sm font-black text-slate-900 dark:text-white">${(run.total_amount_cents / 100).toLocaleString()}</span>
                </div>
              </div>
            )) : (
              <p className="text-slate-400 dark:text-slate-500 text-sm text-center py-8 font-medium">No recent history records found.</p>
            )}
          </div>
        </div>
      </div>
      ...
      {showInitModal && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 md:p-6 bg-slate-950/40 backdrop-blur-md">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 w-full max-w-lg rounded-2xl p-6 md:p-10 shadow-2xl transition-colors">
            <h2 className="text-2xl md:text-3xl font-black text-slate-900 dark:text-white mb-4">Confirm New Run</h2>
            <p className="text-slate-500 dark:text-slate-400 text-sm md:text-base mb-8 md:mb-10 leading-relaxed">You are about to initiate the disbursement process for the period <span className="text-slate-900 dark:text-white font-bold">Oct 01 - Oct 15, 2023</span>. This will notify all approvers.</p>
  ...
  return (
    <div className={`p-6 md:p-8 rounded-2xl border shadow-sm hover:border-primary/40 transition-all ${colors[color] || colors.primary} ${className}`}>
      ...
      <div className="flex flex-wrap items-baseline gap-2 md:gap-3">
        <h3 className="text-3xl md:text-4xl font-black text-slate-900 dark:text-white leading-none transition-colors">{value}</h3>
